  ...
  try {
    ...
    for (const [productId, quantity] of requestedByProduct) {
      const movementResult = await client.query(
        ...
        [
          ...
          sale.id,
          `فاتورة بيع ${input.invoiceNumber}`,
          userId,
        ],
      )
    ...
    if (input.customerId) {
      await insertCustomerLedgerMovement(client, {
        ...
        sourceType: 'sale',
        sourceId: sale.id,
        notes: `فاتورة بيع ${input.invoiceNumber}`,
        userId,
      })
    }
    ...
    for (const payment of paymentBreakdown.value.payments) {
      const savedPayment = await insertIncomingPayment(client, {
        ...
        customerId: input.customerId,
        saleId: sale.id,
        contextLabel: `دفعة فاتورة بيع ${input.invoiceNumber}`,
      ...
      if (input.customerId) {
        await insertCustomerLedgerMovement(client, {
          ...
          notes: `دفعة على فاتورة ${input.invoiceNumber}`,
          userId,
        })
      }
    }
