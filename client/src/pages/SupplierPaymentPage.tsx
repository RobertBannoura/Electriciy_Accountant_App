...
export function SupplierPaymentPage({ defaultStoreId, stores, onDraftStateChange }: {
  ...
  return (
    <section>
      ...
      <div className="mt-6"><SupplierPaymentEditor businessDate={currentBusinessDate()} checks={checks} onChange={setPayments} payments={payments} /></div>
      <div className="mt-6 grid gap-5 rounded-3xl border-2 border-slate-200 bg-white p-5 lg:grid-cols-[1fr_24rem]"><label><span className="mb-2 block font-black">ملاحظات (اختياري)</span><textarea className="min-h-28 w-full rounded-xl border border-slate-300 p-3 text-lg" maxLength={2000} onChange={(event) => setNotes(event.target.value)} value={notes} /></label><div className="rounded-2xl bg-slate-100 p-5"><Row label="الدين قبل الدفع" value={debt} /><Row label="مجموع الدفعة" value={paid} /><Row label="المتبقي" value={remaining} /><button className="mt-5 min-h-16 w-full rounded-2xl bg-violet-700 text-xl font-black text-white disabled:bg-slate-300 disabled:text-slate-600" disabled={!canSave} onClick={() => void save()} type="button">{saving ? 'جارٍ التسجيل…' : 'تسجيل الدفعة'}</button></div></div>
    </section>
  )
}
